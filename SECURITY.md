...
agentcheck is a tool you run on *configuration that may contain secrets*, so its own security posture matters. This document describes that posture and how to report a vulnerability.

> agentcheck is **unofficial** and not affiliated with Anthropic.

## The tool's security model

agentcheck is designed to be safe to run on untrusted input:
...
- **Secrets are redacted** in finding messages — agentcheck reports *that* a secret exists and its kind, not the secret value.
...
agentcheck follows semantic versioning. Security fixes are released for the latest minor of the current major.
...
1. **GitHub Security Advisories** (preferred): open a private report at
   `https://github.com/bacnguyenne/agentcheck/security/advisories/new`.
...
Thank you for helping keep agentcheck and its users safe.
