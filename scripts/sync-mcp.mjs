 ...
 * is validated by agentcheck and only kept if it passes with zero errors.
 *
...
async function npmVersion(pkg) {
  try {
    const r = await fetch(`https://registry.npmjs.org/${pkg}/latest`, { headers: { 'user-agent': 'agentcheck-sync-mcp' } });
    if (!r.ok) return null;
...
async function pypiVersion(pkg) {
  try {
    const r = await fetch(`https://pypi.org/pypi/${pkg}/json`, { headers: { 'user-agent': 'agentcheck-sync-mcp' } });
    if (!r.ok) return null;
...
async function main() {
  ...
  const banner =
    ...
    '// validated by agentcheck (0 errors). Refresh with `npm run sync:mcp`.\n\n';
