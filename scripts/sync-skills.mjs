 ...
 * repositories, validates EACH one with agentcheck itself, and keeps only the
 ...
 *
 * Design goals:
 *  - SAFE: we only ever PARSE fetched content (agentcheck never executes it). The
 ...
 *  - VETTED: an upstream skill is only added if `agentcheck` reports 0 errors.
 *
...
async function fetchSource(src) {
  ...
  try {
    const res = await fetch(url, { headers: { 'user-agent': 'agentcheck-sync-skills' } });
...
const ts =
  ...
  '// A curated catalog of Agent Skills, each VALIDATED by agentcheck (0 errors).\n' +
...
const docLines = [
  ...
  'Each upstream skill is fetched and **validated by agentcheck**; only skills with **0 errors** are added.',
  '',
